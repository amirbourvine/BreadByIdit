...
function LeftPanel({ 
  ...
  // Close "Place New Order" section when panel collapses
  useEffect(() => {
    if (!panelOpen) {
  ...
  // Calculate panel width based on device and state
      ...
      {!isMobile && (
        <button
          ...
          style={{
            ...
            alignItems: 'center',
            justifyContent: 'center',
            width: panelOpen ? '100%' : '40px',
            alignSelf: 'center',
            marginBottom: panelOpen ? '0' : 'auto'
          }}
        >
          {panelOpen ? '◀ Collapse' : '▶'}
        </button>
